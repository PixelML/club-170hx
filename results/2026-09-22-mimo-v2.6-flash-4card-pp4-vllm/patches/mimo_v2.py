...
def _shard_fp8_qkv_proj(
    ...
    for g_idx in range(tp_rank * kv_heads_per_rank, (tp_rank + 1) * kv_heads_per_rank):
        ...
        # pixelml-fix: scale blocks can straddle KV-group boundaries (K spans
        # 1.5 blocks), so per-group scale rows are not
        # s_full.shape[0] // num_kv_heads. Slice the scales by the row range
        # this group covers instead, with a ceil on the closing block.
        scale_row_start = row_start // block
        scale_row_end = (row_start + rows_per_group + block - 1) // block
        # Dequantize this group's weights.
        w_g = w_full[row_start : row_start + rows_per_group].to(torch.float32)
        s_g = s_full[scale_row_start:scale_row_end].to(torch.float32)
        offset = row_start - scale_row_start * block
        s_g_expanded = s_g.repeat_interleave(block, dim=0).repeat_interleave(
            block, dim=1
        )[offset : offset + rows_per_group]
