...
def prepare_moe_mxfp4_layer_for_marlin(
    ...
    # bounds (illegal memory access). Detect and return the packed state.
    if w13.dtype == torch.int32 and w2.dtype == torch.int32:
        logger.info_once(
    ...
    import torch as _t
    _t.cuda.synchronize()
    logger.info("pixelml-step: repack w13 begin")
    w13 = repack_weight(w13, "w13")
    _t.cuda.synchronize()
    logger.info("pixelml-step: repack w13 done")
    w2 = repack_weight(w2, "w2")
    _t.cuda.synchronize()
    logger.info("pixelml-step: repack w2 done")
    ...
    _t.cuda.synchronize()
    logger.info("pixelml-step: permute w13 scales begin")
    w13_scale = permute_scales(w13_scale, "w13")
    _t.cuda.synchronize()
    logger.info("pixelml-step: permute w13 scales done")
    _t.cuda.synchronize()
    logger.info("pixelml-step: permute w2 scales begin")
    w2_scale = permute_scales(w2_scale, "w2")
    _t.cuda.synchronize()
    logger.info("pixelml-step: permute w2 scales done")
