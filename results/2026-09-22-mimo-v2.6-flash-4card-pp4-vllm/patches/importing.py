...
if HAS_TRITON:
    try:
        ...
        if is_distributed_env and len(active_drivers) == 0:
            ...
            # pixelml-fix: triton 3.7 CudaDriver.is_active() returns False at
            # import time (pre-CUDA-init) and vLLM then disables Triton even
            # though the driver lazily activates fine. Probe the active
            # driver directly; keep Triton enabled when CUDA is real.
            try:
                import torch as _torch
                from triton.runtime import driver as _drv

                if len(active_drivers) == 0 and _torch.cuda.is_available():
                    _d = _drv.active
                    if _d is not None:
                        active_drivers = [_d]
                        logger.info(
                            "pixelml-fix: triton driver lazily activated "
                            "after import-time detection found 0."
                        )
            except Exception as _e:
                logger.debug("pixelml-fix triton lazy probe failed: %s", _e)
