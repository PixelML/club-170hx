...
|---|---|---|---|---|
| 1 | Host-vs-container path mismatch (3x) | A patch applied on the host was not bind-mounted into the running container, so the container kept executing stale code; a research-path bind mount also pointed at the wrong directory once | Add or correct the explicit `-v host:container:ro` mount, then verify the file content from inside the container, not just on the host | ~15-30 min per occurrence |
...
| 3 | NVRM VA-space corruption after an OOM kill storm | `torch.cuda.is_available()` returns False host-wide even though device count reads correctly | Unload and reload the NVIDIA kernel modules (`nvidia_uvm`, then `nvidia`) — restores every GPU with no VM reboot; confirmed working twice | ~10-15 min recovery vs. hours for a reboot |
...
| 8 | Missing required keyword argument in a patched constructor | A patched attention-indexer class was missing a required constructor argument; only surfaces at runtime | Run an import smoke test plus an undefined-name/lint scan on every patched file, not just a syntax check | One extra review round-trip |
| 9 | A weight-loader fix landed in the model's loader but not in its speculative draft model's separate loader | The draft/speculative model loads weights through its own code path that silently missed the same fix | Apply loader fixes to every code path that touches the same tensors, not just the primary one | One follow-up patch cycle |
...
- **Module-reload recovery sequence for NVRM VA-space corruption:** unload the `nvidia_uvm` and `nvidia` kernel modules, then reload them — restores every GPU without a VM reboot. Confirmed working twice.
...
- **A local NVMe staging pipeline:** hash the source, copy, verify the destination against a frozen manifest — cutting cold-start weight availability from 30+ minutes over the network to about two minutes.
- **A container-scoped read-only network mount with no host-level privilege change:** when a target host has no passwordless elevated access but its container runtime needs none, run a privileged container from the runtime image, install the network-filesystem client tools inside that container only, then mount the canonical export read-only inside the container's own mount namespace. Zero host configuration changes, zero bytes copied, checkpoint reachable read-only.
...
- **A parallel review/approval process ran continuously with unclear value.** A paired review process mirrored essentially every action across thousands of turns and approved nearly all of them; no case was found in this pass of it meaningfully altering an action. Worth auditing whether a lighter-weight review process would catch the same issues before continuing to run a full-mirror review process unconditionally.
