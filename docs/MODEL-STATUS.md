...
| Qwen3.8-Flash-Next | AWQ W4A16 (`wtdcode/Qwen3.8-Flash-Next-AWQ-W4A16`) | vLLM 0.11.0 sm80 backport + PLE-PP patch, PP2, MTP k=3, PLE on CPU offload | 2 (bare metal, no power cap) | Measured (community report, not this club's own run) | **87.8 tok/s c=1 decode**, 351.7 tok/s aggregate at c=8, prefill 5,897 tok/s at ~15k tokens; PP2 beat TP2+EP on every metric (TP2: 69.7 / 223.6 / 1,274); single warm run per config, no medians | [Issue #59](https://github.com/PixelML/club-170hx/issues/59) |
...
model is a natural first test once four 64 GiB cards are locally available. A community
member has since served it on two cards bare metal with PLE on CPU offload,
which confirms the bare-metal half of that expectation
([issue #59](https://github.com/PixelML/club-170hx/issues/59); see the row above).
