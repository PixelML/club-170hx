...
| Qwen3.8-27B | NVFP4 | vLLM | 1 (x3 runs, 180 W) | Pending evidence repair | Not presented — receipts unmerged | [repo PR 1](https://github.com/PixelML/Qwen3.8-27B-CMP-170HX/pull/1) |
...
| DeepSeek-V4-Flash-Vision-Exp | FP8 e4m3, 48 shards | SM80 vLLM fork, PP4, layer partition 11,11,11,10, DSpark k=6 (text path) | 4 | Measured; benchmark re-run in progress | Decode c=1 `{{C1}}` tok/s; aggregate c=4 `{{C4}}` tok/s; aggregate c=16 `{{C16}}` tok/s; uncached prefill (2,941 tokens) `{{PREFILL}}` tok/s; warm TTFT `{{TTFT}}` s. Earlier superseded ladder: 101.21 tok/s @ c=1, 169.65 tok/s @ c=4, 325.5 tok/s prefill | [PixelML/DeepSeek-V4-Flash-Vision-Exp-CMP-170HX](https://github.com/PixelML/DeepSeek-V4-Flash-Vision-Exp-CMP-170HX) |
