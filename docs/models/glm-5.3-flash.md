...
- **2026-09-06 (measurement note)** — Fixed a bug in `bench_glm53.py`'s decode harness: `median_tok_s_warm` used `sorted(values)[len(values)//2]`, which is not a median for an even-sized sample (the 4 warm reps) — it read 80.66 tok/s for k=3 where the true warm median is 74.28. Every number this page publishes was already computed as the median of all 5 reps (the declared P2 protocol), so **no headline figure on this page changes**; the harness now uses `statistics.median` throughout so future boots don't regenerate the wrong field.
